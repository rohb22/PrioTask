...
import { ScoredTask } from '../lib/sorting';
...
function urgencyColor(bucket: ScoredTask['urgencyBucket']): string {
  switch (bucket) {
...
export function TaskCard({ scoredTask, allTasks, onProgressUpdate, onPress }: Props) {
  const { task, urgencyBucket } = scoredTask;
  ...
  return (
    <Animated.View style={[styles.card, { borderLeftColor: urgencyColor(urgencyBucket) }, animStyle]}>
